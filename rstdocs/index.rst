

.. raw:: html

   <div style="height: 0; visibility: hidden;">

Home
========

.. raw:: html

   </div>

.. figure::  _static/img/rivdark55c.png
    :class: dark-light
    :scale: 20%
    :align: center
    :alt: rivt logo
    
.. raw:: html

   <hr>

*rivt* is an open source software project for writing engineering documents.
The documents are generated from text files (:term:`rivt files`) 
as formatted text, HTML and PDF documents (:term:`rivt docs`).

A *rivt file* is a :term:`Python` file (.py) that imports the :term:`rivtlib`
Python package and interprets :term:`rivt markup`. The same *rivt file* can
publish documents in each format, and be compiled with other documents into a
:term:`rivt report`. This site is an example of a report.

Small *rivt file* examples are provided :doc:`here <rvD05-docex>` and 
:doc:`here<rvD06-reportex>`. An interface for searching *rivt files* 
on *GitHub* is :doc:`here <rvE03-ghsearch>`.

.. raw:: html

    <hr>

.. toctree::
    :maxdepth: 1
    :hidden:

    rvA01-intro.rst
    rvB01-install.rst
    rvC01-markup.rst
    rvD01-documents.rst
    rvE01-collab.rst
    rvF01-quickref.rst